...
async function notion(path, options = {}) {
  ...
  const response = await fetch(base + path, { ...options, headers: { Authorization: `Bearer ${process.env.NOTION_API_KEY}`, 'Notion-Version': NOTION_VERSION, 'Content-Type': 'application/json', ...(options.headers || {}) } });
...
async function resolveAgentId() {
  ...
  const list = await notion('/v1/agents/query', { method: 'POST', body: JSON.stringify({ query: name, page_size: 10 }) });
...
export default async function handler(req, res) {
  ...
  try {
    const payload = sessionId ? { session_id: sessionId, message } : { agent_id: await resolveAgentId(), message };
    const session = await notion('/v1/sessions', { method: 'POST', body: JSON.stringify(payload) });
    const id = session.id || sessionId; let status = session.status;
    for (let i = 0; i < 30 && ['queued', 'in_progress'].includes(status); i++) { await sleep(1000); status = (await notion(`/v1/sessions/${id}`)).status; }
    if (status !== 'completed') throw new Error(`Agent session ended with status: ${status}`);
    const events = await notion(`/v1/sessions/${id}/events/query`, { method: 'POST', body: JSON.stringify({ filter: { property: 'type', event_type: { equals: 'agent.message' } } }) });
  ...
  } catch (error) { return res.status(500).json({ error: error.message || 'Unable to reach Notion Agent.' }); }
}
